...
export default async function BlueprintPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white text-black pt-24 px-4 pb-12 selection:bg-[#C5A059]/30 selection:text-black relative overflow-x-hidden">
      {/* Background Glows */}
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <Link href="/library" className="inline-flex items-center gap-2 text-gray-500 hover:text-[#C5A059] mb-8 transition-colors group">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="flex-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#C5A059]/10 border border-[#C5A059]/20 text-[#C5A059] text-[10px] font-black tracking-widest uppercase mb-4">
              {blueprint.category} Blueprint
            </div>
            <h1 className="text-4xl md:text-5xl font-serif font-bold tracking-tight mb-4 ">{blueprint.title}</h1>
            <p className="text-gray-600 font-light text-lg leading-relaxed">{blueprint.overview}</p>
          </div>
          <div className="bg-black/5 border border-black/10 p-8 rounded-3xl text-center min-w-[200px] backdrop-blur-xl">
             <div className="text-[10px] font-black tracking-widest text-gray-500 uppercase mb-2">Difficulty</div>
             <div className="text-3xl font-black text-[#C5A059] tracking-tighter ">{blueprint.difficulty}</div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
           <div className="p-6 rounded-2xl bg-white shadow-sm hover:shadow-2xl border border-black/5">
              ...
              <div className="font-bold text-lg text-black flex items-center gap-2">
                <CircleDollarSign className="w-4 h-4 text-[#C5A059]" /> {blueprint.startupCost}
              </div>
           </div>
           <div className="p-6 rounded-2xl bg-white shadow-sm hover:shadow-2xl border border-black/5">
              <div className="text-gray-500 text-[10px] font-black tracking-widest uppercase mb-2">Time to Profit</div>
              <div className="font-bold text-lg text-[#C5A059] flex items-center gap-2">
        ...
        <div className="grid md:grid-cols-2 gap-12">
           <div className="space-y-8">
              <section>
                 <h2 className="text-xl font-black mb-6 flex items-center gap-2 ">
                   <Target className="text-[#C5A059] w-5 h-5" /> Execution Steps
                 </h2>
                 <ul className="space-y-4">
                    {blueprint.steps.map((step: string, i: number) => (
                      <li key={i} className="flex gap-4 items-start text-gray-600 font-light text-sm leading-relaxed">
                         <div className="w-6 h-6 rounded-lg bg-[#C5A059]/10 flex-shrink-0 flex items-center justify-center text-[10px] font-black text-[#C5A059] border border-[#C5A059]/20">
                           {i+1}
                         </div>
                         {step}
                      </li>
           ...
           <div className="space-y-6">
              <div className="p-8 rounded-3xl bg-gradient-to-br from-[#C5A059]/10 to-transparent border border-[#C5A059]/20 relative overflow-hidden group">
                 <div className="relative z-10">
                    <h3 className="text-xl font-black mb-4 ">Required Toolkit</h3>
                    <ul className="space-y-3 mb-8">
                      {blueprint.tools.map((tool, i) => (
                        <li key={i} className="flex items-center gap-3 text-sm text-gray-600 font-light">
                          <Hammer className="w-4 h-4 text-[#C5A059]" /> {tool}
                        </li>
                      ))}
                    </ul>
                    <button className="w-full py-4 bg-[#C5A059] text-black font-black rounded-xl hover:shadow-[0_0_20px_rgba(255,215,0,0.4)] transition-all flex items-center justify-center gap-2 group">
              ...
              <div className="p-6 rounded-2xl bg-white shadow-sm hover:shadow-2xl border border-black/5">
                 <h3 className="font-bold flex items-center gap-2 text-sm mb-4 text-gray-600 font-light">
                   <Info className="w-4 h-4 text-gray-500" /> pSEO Metadata
                 </h3>
                 <div className="flex flex-wrap gap-2">
                    {blueprint.pSEOKeywords.map((tag, i) => (
                      <span key={i} className="text-[9px] font-black text-gray-600 uppercase tracking-widest bg-black/5 px-2 py-1 rounded-md">
                        #{tag.replace(/\s+/g, '')}
                      </span>
                    ))}
                 </div>
        ...
        <div className="mt-20 pt-12 border-t border-black/5 text-center">
           <h2 className="text-2xl font-black mb-4  tracking-tight">Ready to stop dreaming?</h2>
           <p className="text-gray-600 font-light mb-8 max-w-xl mx-auto text-sm">
           ...
           <Link href="/#join" className="px-8 py-4 bg-black/5 border border-black/10 hover:bg-black/10 rounded-xl font-black transition-all text-sm uppercase tracking-widest">
             Join the Network
           </Link>
        </div>
      </div>
